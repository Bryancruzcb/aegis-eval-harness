...
from aegis_eval.core.evaluators import JudgeParseError, render_transcript, run_llm_judge_eval_conversation
from aegis_eval.core.lock import exclusive_run
from aegis_eval.core.providers import ProviderError
from aegis_eval.workflows.grader_quality.quality_data import cohorts, require_local
from aegis_eval.workflows.grader_quality.safety_grader import BENIGN_PROMPT, HARM_PROMPT, SafetyGrader, SafetyJudgment

ROOT = config.BASE_DIR
OUT = ROOT / 'output' / 'grader-quality' / comparison.PROVENANCE_VERSION
SOURCE_MODULES = {
    'quality_eval.py': 'aegis_eval.workflows.grader_quality.quality_eval',
    'quality_data.py': 'aegis_eval.workflows.grader_quality.quality_data',
    'safety_grader.py': 'aegis_eval.workflows.grader_quality.safety_grader',
    'refusal_grader.py': 'aegis_eval.harness.refusal_grader',
}
...
def main():
    ...
    if result == 0 and args.cohort == 'all':
        from aegis_eval.workflows.grader_quality.quality_report import write_report
        write_report()
    return result
